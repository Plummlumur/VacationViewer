...
sudo -u "$APP_USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; $VENV_DIR/bin/python manage.py makemigrations"
sudo -u "$APP_USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; $VENV_DIR/bin/python manage.py migrate"
echo "  Collecting static files..."
sudo -u "$APP_USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; $VENV_DIR/bin/python manage.py collectstatic --no-input"
