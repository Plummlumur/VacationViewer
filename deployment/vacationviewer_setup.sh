...
cd "$APP_DIR"
sudo -u "$USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; $VENV_DIR/bin/python manage.py makemigrations"
sudo -u "$USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; $VENV_DIR/bin/python manage.py migrate"
sudo -u "$USER" bash -c "cd $APP_DIR && export PYTHONPATH=$APP_DIR && set -a; source $ENV_FILE; set +a; DJANGO_SETTINGS_MODULE=vacationviewer.settings $VENV_DIR/bin/python manage.py collectstatic --no-input || true"
